...
void APP_Initialize ( void )
{
    ...
    char unsigned check[1];
    I2C_read_multiple(SLAVE_ADDR, 0x0F, check, 1);
    char r = check[0];
...
//                    vector ++;
//                    char message[200];
                    char unsigned data[14];
//                    short temp;
//
                    I2C_read_multiple(SLAVE_ADDR, 0x20, data, 14);
//                    temp = (data[3] << 8) | data[2];
//                    sprintf(message, "X-ang: %d", temp);
//                    writeToLCD(message, 10, 10);
//                    temp = (data[5] << 8) | data[4];
//                    sprintf(message, "Y-ang: %d", temp);
//                    writeToLCD(message, 10, 20);
                    appData.xCoordinate = (int8_t)((data[3] << 8) | data[2])/20;
                    appData.yCoordinate = (int8_t)((data[5] << 8) | data[4])/20;
